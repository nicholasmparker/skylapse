...
def test_focus_score_with_mock_capture(monkeypatch, tmp_path: Path):
    ...
    # Capture once to generate an image
    r_cap = client.post("/api/admin/capture_once", headers=_auth_headers("test-token"))
    ...
    r = client.get("/api/admin/focus_score", headers=_auth_headers("test-token"))
